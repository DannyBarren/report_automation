...
class SectionFrame(BaseModel):
    """One still-frame of evidence within a section (multi-mark / multi-photo support).

    A section can now hold **many** frames — one per MARK tap or spoken cue the inspector made
    in that area — instead of a single image. Each frame carries its own capture time, the
    narration snippet nearest that moment, and how it was captured, so the PDF can render a
    labeled gallery and the writer can reason over every piece of evidence.
    """

    timestamp_sec: float = Field(default=0.0, description="Anchor time for this frame (seconds).")
    image_timestamp_sec: float = Field(
        default=0.0, description="Time used to extract this still (seconds)."
    )
    narration: str = Field(default="", description="Narration snippet nearest this frame.")
    context_narration: str = Field(
        default="",
        description="Richer narration window around the mark (default 5s before → 10s after).",
    )
    context_start_sec: float = Field(default=0.0, description="Start of the context window (seconds).")
    context_end_sec: float = Field(default=0.0, description="End of the context window (seconds).")
    source: str = Field(
        default="manual_mark",
        description="How it was captured: manual_mark | spoken_cue | step_window.",
    )
    note: str = Field(default="", description="Inspector's typed note for this mark, if any.")
    image_path: str | None = Field(default=None, description="Filesystem path to extracted still.")
    image_uri: str | None = Field(default=None, description="file:// URI for WeasyPrint <img>.")
    caption: str = Field(default="", description="Optional precomputed caption.")
...
class MatchedSection(BaseModel):
    ...
    ``frames`` holds **every** evidence still for the section (one per MARK tap / spoken cue).
    ``timestamp_sec`` / ``image_timestamp_sec`` mirror the first frame for backward compatibility.
    """
    ...
    frames: list[SectionFrame] = Field(
        default_factory=list,
        description="All evidence frames for this section (multi-mark support).",
    )
...
class ReportSectionOutput(BaseModel):
    ...
    frames: list[SectionFrame] = Field(
        default_factory=list,
        description="All evidence stills for this section; empty on legacy reports (single image).",
    )
