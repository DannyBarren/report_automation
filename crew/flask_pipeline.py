...
def run_v2_pipeline(
    ...
    *,
    ...
    ``mark_events`` are explicit timestamps captured by the recorder's manual "MARK THIS
    SECTION" button. They are forwarded to seed frame timestamps when transcript-based
    "Mark this." matching yields nothing, improving section→frame alignment.

    ``step_events`` are the step boundaries of the single continuous recording (when each
    guided step began). They give the deterministic matcher an accurate per-section time
    window so narration is aligned to the right section even when a spoken cue was missed.
    """
    templates = json.loads(templates_json)
    log_job_step(
        video_id,
        "v2",
        f"start path={video_path.name} marks={len(mark_events or [])} steps={len(step_events or [])}",
    )
    ...
    try:
        ...
        # Even in a catastrophic failure we MUST preserve the inspector's marks + step windows
        # (and any transcript we can still get). Passing them means the fallback report anchors
        # each section to its real MARK timestamps and narration windows — instead of the old
        # behavior that rebuilt from an EMPTY transcript with no marks, which made every section
        # read "not reached" with a single 0.0s (video-start) frame.
        fallback_transcript = empty_transcript_payload()
        try:
            from crew.jobdoc_crew_v2 import transcribe_video_file

            fallback_transcript = transcribe_video_file(
                video_path, on_progress=on_progress, allow_empty=True
            )
        except Exception:  # noqa: BLE001 — transcription is best-effort here
            logger.warning("Fallback transcription unavailable — using marks/steps only.")
