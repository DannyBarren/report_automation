...
Real speech-to-text feeds ``build_matched_sections``. Crew agents run with **live** OpenAI
...
class JobDocCrew:
    ...
    def kickoff(self, inputs: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        mark_events = list(inputs.get("mark_events") or self.mark_events or [])
        step_events = list(inputs.get("step_events") or self.step_events or [])
        matched: list[MatchedSection] = (
            build_matched_sections(tr, templates, mark_events=mark_events, step_events=step_events)
            if templates
            else []
        )
        ...
        # Single shared production reasoning model (OpenAI, e.g. gpt-4o) for all crew agents.
